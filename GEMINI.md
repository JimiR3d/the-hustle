...
- Soft theatrical spotlight focus with 3D Zoom In: clean spotlighted card smoothly zooms forward (`scale(1.22) translateY(-20px)`) without outer yellow glow outlines, soft gentle dark overlay (`rgba(0, 0, 0, 0.28); backdrop-filter: brightness(0.82);`), with header logo and game timer remaining 100% bright above the dimmer.
