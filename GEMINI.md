# Ellis Game Show Board

## Overview
A single-page digital game show board application designed for arena displays (1920x1080) and admin show controls. Built around a 2-player team card format inspired by "THE HUSTLE Love & MONEY". Features real-time state synchronization across dual screens (Main Arena Display and Admin Control Screen) via BroadcastChannel and LocalStorage.
...
- **State & Sync:** BroadcastChannel API + LocalStorage dual-channel sync engine with transaction deduplication
...
│   └── assets/          # Custom assets (logo.png, background.png, back_glass.png, score_board.png, Players/*.png)
```
...
## Current State
- **Status:** Complete 2-Player Team MVP with 1920x1080 Arena display, Host Admin controller with live player reshuffling dropdowns, dual-channel zero-latency state sync, explicit Start/Pause/Stop/Reset timer controls, Web Audio spy countdown buzzer sound, line-free edge-to-edge background fitting, and textless card tearing splits.
